    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                func(self, **kwargs)
                if self.timerOn:
                    execTime = time.perf_counter() - timerStart
...
class G2CmdShell(cmd.Cmd, object):
    def __init__(self, debug, hist_disable):
        ...
        self.__hidden_methods = (
            ...
            "do_getRedoRecord",
            "do_getEntityDetails",
            "do_getRelationshipDetails",
            "do_getEntityResume",
            "do_getEntityListBySize",
            "do_getDataSourceCounts",
            "do_getMappingStatistics",
            "do_getGenericFeatures",
            "do_getEntitySizeBreakdown",
            "do_getFeature",
            "do_getResolutionStatistics",
            "do_findEntitiesByFeatureIDs",
            "do_getPhysicalCores",
            "do_getLogicalCores",
            "do_getTotalSystemMemory",
            "do_getAvailableMemory",
            "do_exportTokenLibrary",
            "do_hashRecord",
            "do_hashFile",
            "do_process",
            "do_processWithInfo",
        ...
        self.initialized = (
            self.restart
        ) = self.restart_debug = self.quit = self.timerOn = False
        ...
        if not self.docker_launched:
            ...
            if config_exists:
                try:
                    ...
                    config_json_format = self.config["FORMATTING"]["JsonFormat"]
                    config_color_output = self.config["FORMATTING"].getboolean(
                        "ColorOutput"
                    )
                    self.json_output_format = config_json_format
                    self.json_output_color = config_color_output
        ...
        # Command parsers
        self.parser = G2CommandArgumentParser(
            add_help=False, prog="G2Command", usage=argparse.SUPPRESS
        )
        self.subparsers = self.parser.add_subparsers()

        hashRecord_parser = self.subparsers.add_parser(
            "hashRecord", usage=argparse.SUPPRESS
        )
        ...
        processWithInfo_parser = self.subparsers.add_parser(
            "processWithInfo", usage=argparse.SUPPRESS
        )
        ...
        searchByAttributesV3_parser = self.subparsers.add_parser(
            "searchByAttributesV3", usage=argparse.SUPPRESS
        )
        searchByAttributesV3_parser.add_argument("jsonData")
        searchByAttributesV3_parser.add_argument("searchProfile")
        searchByAttributesV3_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        validateLicenseFile_parser = self.subparsers.add_parser(
            "validateLicenseFile", usage=argparse.SUPPRESS
        )
        validateLicenseFile_parser.add_argument("licenseFilePath")

        validateLicenseStringBase64_parser = self.subparsers.add_parser(
            "validateLicenseStringBase64", usage=argparse.SUPPRESS
        )
        validateLicenseStringBase64_parser.add_argument("licenseString")

        hashFile_parser = self.subparsers.add_parser(
            "hashFile", usage=argparse.SUPPRESS
        )
        hashFile_parser.add_argument("inputFile")
        hashFile_parser.add_argument("-o", "--outputFile", required=False)

        exportEntityJson_parser = self.subparsers.add_parser(
        ...
        exportEntityCsv_parser = self.subparsers.add_parser(
        ...
        exportEntityCsv_parser.add_argument(
            "-t", "--headersForCSV", required=False, type=str
        )

        recordModify_parser = self.subparsers.add_parser(
            "recordModify", usage=argparse.SUPPRESS
        )
        ...
        addRecordWithInfo_parser = self.subparsers.add_parser(
            "addRecordWithInfo", usage=argparse.SUPPRESS
        )
        ...
        addRecordWithInfo_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )

        addRecordWithReturnedRecordID_parser = self.subparsers.add_parser(
            "addRecordWithReturnedRecordID", usage=argparse.SUPPRESS
        )
        addRecordWithReturnedRecordID_parser.add_argument("dataSourceCode")
        addRecordWithReturnedRecordID_parser.add_argument("recordID")
        addRecordWithReturnedRecordID_parser.add_argument("jsonData")

        addRecordWithInfoWithReturnedRecordID_parser = self.subparsers.add_parser(
            "addRecordWithInfoWithReturnedRecordID", usage=argparse.SUPPRESS
        )
        addRecordWithInfoWithReturnedRecordID_parser.add_argument("dataSourceCode")
        addRecordWithInfoWithReturnedRecordID_parser.add_argument("recordID")
        addRecordWithInfoWithReturnedRecordID_parser.add_argument("jsonData")
        addRecordWithInfoWithReturnedRecordID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        deleteRecordWithInfo_parser = self.subparsers.add_parser(
            "deleteRecordWithInfo", usage=argparse.SUPPRESS
        )
        deleteRecordWithInfo_parser.add_argument("dataSourceCode")
        deleteRecordWithInfo_parser.add_argument("recordID")
        deleteRecordWithInfo_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        findPathExcludingByEntityID_parser = self.subparsers.add_parser(
            "findPathExcludingByEntityID", usage=argparse.SUPPRESS
        )
        ...
        findPathExcludingByEntityID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )

        findPathIncludingSourceByEntityID_parser = self.subparsers.add_parser(
            "findPathIncludingSourceByEntityID", usage=argparse.SUPPRESS
        )
        ...
        findPathIncludingSourceByEntityID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        getEntityDetails_parser = self.subparsers.add_parser(
            "getEntityDetails", usage=argparse.SUPPRESS
        )
        getEntityDetails_parser.add_argument("entityID", type=int)
        getEntityDetails_parser.add_argument(
            "-i",
        ...
        getRelationshipDetails_parser = self.subparsers.add_parser(
            "getRelationshipDetails", usage=argparse.SUPPRESS
        )
        getRelationshipDetails_parser.add_argument("relationshipID", type=int)
        getRelationshipDetails_parser.add_argument(
            "-i",
            "--includeInternalFeatures",
            action="store_true",
            required=False,
        ...
        getMappingStatistics_parser = self.subparsers.add_parser(
            "getMappingStatistics", usage=argparse.SUPPRESS
        )
        getMappingStatistics_parser.add_argument(
            "-i",
            "--includeInternalFeatures",
            action="store_true",
            required=False,
        ...
        checkDBPerf_parser = self.subparsers.add_parser(
            "checkDBPerf", usage=argparse.SUPPRESS
        )
        checkDBPerf_parser.add_argument("secondsToRun", type=int, nargs="?", default=3)

        getGenericFeatures_parser = self.subparsers.add_parser(
            "getGenericFeatures", usage=argparse.SUPPRESS
        )
        getGenericFeatures_parser.add_argument("featureType")
        getGenericFeatures_parser.add_argument(
            "-m", "--maximumEstimatedCount", required=False, type=int, default=1000
        )

        getEntitySizeBreakdown_parser = self.subparsers.add_parser(
            "getEntitySizeBreakdown", usage=argparse.SUPPRESS
        )
        getEntitySizeBreakdown_parser.add_argument("minimumEntitySize", type=int)
        getEntitySizeBreakdown_parser.add_argument(
            "-i",
            "--includeInternalFeatures",
            action="store_true",
            required=False,
        ...
        getEntityResume_parser = self.subparsers.add_parser(
            "getEntityResume", usage=argparse.SUPPRESS
        )
        getEntityResume_parser.add_argument("entityID", type=int)

        getEntityListBySize_parser = self.subparsers.add_parser(
            "getEntityListBySize", usage=argparse.SUPPRESS
        )
        ...
        reevaluateRecordWithInfo_parser = self.subparsers.add_parser(
            "reevaluateRecordWithInfo", usage=argparse.SUPPRESS
        )
        reevaluateRecordWithInfo_parser.add_argument("dataSourceCode")
        reevaluateRecordWithInfo_parser.add_argument("recordID")
        reevaluateRecordWithInfo_parser.add_argument(
            "-f", "--flags", required=False, type=int
        )
        ...
        reevaluateEntityWithInfo_parser = self.subparsers.add_parser(
            "reevaluateEntityWithInfo", usage=argparse.SUPPRESS
        )
        reevaluateEntityWithInfo_parser.add_argument("entityID", type=int)
        reevaluateEntityWithInfo_parser.add_argument(
            "-f", "--flags", required=False, type=int
        )
        ...
        findPathExcludingByRecordID_parser = self.subparsers.add_parser(
            "findPathExcludingByRecordID", usage=argparse.SUPPRESS
        )
        ...
        findPathExcludingByRecordID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )

        findPathIncludingSourceByRecordID_parser = self.subparsers.add_parser(
            "findPathIncludingSourceByRecordID", usage=argparse.SUPPRESS
        )
        ...
        findPathIncludingSourceByRecordID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        whyEntityByRecordID_parser = self.subparsers.add_parser(
            "whyEntityByRecordID", usage=argparse.SUPPRESS
        )
        whyEntityByRecordID_parser.add_argument("dataSourceCode")
        whyEntityByRecordID_parser.add_argument("recordID")
        whyEntityByRecordID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )

        whyEntityByEntityID_parser = self.subparsers.add_parser(
            "whyEntityByEntityID", usage=argparse.SUPPRESS
        )
        whyEntityByEntityID_parser.add_argument("entityID", type=int)
        whyEntityByEntityID_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        exportConfig_parser = self.subparsers.add_parser(
            "exportConfig", usage=argparse.SUPPRESS
        )
        exportConfig_parser.add_argument("-o", "--outputFile", required=False)

        exportTokenLibrary_parser = self.subparsers.add_parser(
            "exportTokenLibrary", usage=argparse.SUPPRESS
        )
        exportTokenLibrary_parser.add_argument("-o", "--outputFile", required=False)

        findEntitiesByFeatureIDs_parser = self.subparsers.add_parser(
            "findEntitiesByFeatureIDs", usage=argparse.SUPPRESS
        )
        ...
        purgeRepository_parser.add_argument(
            "-n", "--noReset", required=False, action="store_true", default=False
        )
        purgeRepository_parser.add_argument(
            "-FORCEPURGE",
            "--FORCEPURGE",
        ...
        processRedoRecordWithInfo_parser = self.subparsers.add_parser(
            "processRedoRecordWithInfo", usage=argparse.SUPPRESS
        )
        processRedoRecordWithInfo_parser.add_argument(
            "-f", "--flags", required=False, nargs="+"
        )
        ...
        setOutputColor_parser.add_argument("outputColor", nargs="?")
    ...
    def preloop(self):
        ...
        try:
            ...
            self.g2_hasher.initWithConfig(
                "pyG2Hasher", g2module_params, exportedConfig, self.debug_trace
            )
    ...
    def do_processWithInfo(self, **kwargs):
        ...
        self.g2_engine.processWithInfo(
            kwargs["parsed_args"].jsonData, kwargs["response"], **kwargs["flags_int"]
        )
    ...
    def do_exportCSVEntityReport(self, **kwargs):
        ...
        try:
            with open(kwargs["parsed_args"].outputFile, "w") as data_out:
                exportHandle = self.g2_engine.exportCSVEntityReport(
                    kwargs["parsed_args"].headersForCSV, **kwargs["flags_int"]
                )
    ...
    def do_exportJSONEntityReport(self, **kwargs):
        ...
        try:
            with open(kwargs["parsed_args"].outputFile, "w") as data_out:
                exportHandle = self.g2_engine.exportJSONEntityReport(
                    **kwargs["flags_int"]
                )
                rowData = self.g2_engine.fetchNext(exportHandle, response)
                recCnt = 0
    ...
    def do_getConfig(self, **kwargs):
        ...
        self.g2_configmgr.getConfig(kwargs["parsed_args"].configID, kwargs["response"])
        self.printResponse(kwargs["response"])
    ...
    def do_addConfigFile(self, **kwargs):
        ...
        self.g2_configmgr.addConfig(
            config, kwargs["parsed_args"].configComments, kwargs["response"]
        )
        self.printResponse(
            f'Configuration added, ID = {kwargs["response"].decode()}', "success"
        )
    ...
    def do_replaceDefaultConfigID(self, **kwargs):
        ...
        self.g2_configmgr.replaceDefaultConfigID(
            kwargs["parsed_args"].oldConfigID, kwargs["parsed_args"].newConfigID
        )
    ...
    def do_addRecordWithReturnedRecordID(self, **kwargs):
        ...
        Syntax:
            addRecordWithReturnedRecordID DSRC_CODE RECORD_ID JSON_DATA

        Example:
            addRecordWithReturnedRecordID test 1001 '{"NAME_FULL":"Robert Smith", "DATE_OF_BIRTH":"7/4/1976", "PHONE_NUMBER":"787-767-2088"}'
    ...
    def do_addRecordWithInfoWithReturnedRecordID(self, **kwargs):
        ...
        Example:
            addRecordWithInfoWithReturnedRecordID test 1001 '{"NAME_FULL":"Robert Smith", "DATE_OF_BIRTH":"7/4/1976", "PHONE_NUMBER":"787-767-2088"}'
    ...
    def do_reevaluateEntity(self, **kwargs):
        ...
        self.g2_engine.reevaluateEntity(
            kwargs["parsed_args"].entityID, **kwargs["flags_int"]
        )
    ...
    def do_reevaluateEntityWithInfo(self, **kwargs):
        ...
        self.g2_engine.reevaluateEntityWithInfo(
            kwargs["parsed_args"].entityID, kwargs["response"], **kwargs["flags_int"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_deleteRecord(self, **kwargs):
        ...
        self.g2_engine.deleteRecord(
            kwargs["parsed_args"].dataSourceCode, kwargs["parsed_args"].recordID
        )
    ...
    def do_getVirtualEntityByRecordID(self, **kwargs):
        ...
        self.g2_engine.getVirtualEntityByRecordID(
            kwargs["parsed_args"].recordList, kwargs["response"], **kwargs["flags_int"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_processRedoRecordWithInfo(self, **kwargs):
        ...
        self.g2_engine.processRedoRecordWithInfo(
            kwargs["response"], kwargs["withInfo"], **kwargs["flags_int"]
        )
    ...
    def do_getEntityResume(self, **kwargs):
        ...
        self.g2_diagnostic.getEntityResume(
            kwargs["parsed_args"].entityID, kwargs["response"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_getEntityListBySize(self, **kwargs):
        ...
        sizedEntityHandle = self.g2_diagnostic.getEntityListBySize(
            kwargs["parsed_args"].entitySize
        )
        response = bytearray()
        ...
        while rowData:
            ...
            response = bytearray()
            rowData = self.g2_diagnostic.fetchNextEntityBySize(
    ...
    def do_checkDBPerf(self, **kwargs):
        ...
        self.g2_diagnostic.checkDBPerf(
            kwargs["parsed_args"].secondsToRun, kwargs["response"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_getMappingStatistics(self, **kwargs):
        ...
        self.g2_diagnostic.getMappingStatistics(
            kwargs["parsed_args"].includeInternalFeatures, kwargs["response"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_getFeature(self, **kwargs):
        ...
        self.g2_diagnostic.getFeature(
            kwargs["parsed_args"].featureID, kwargs["response"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_findEntitiesByFeatureIDs(self, **kwargs):
        ...
        self.g2_diagnostic.findEntitiesByFeatureIDs(
            kwargs["parsed_args"].features, kwargs["response"]
        )
        self.printResponse(kwargs["response"])
    ...
    def do_validateLicenseFile(self, **kwargs):
        ...
        returnCode = self.g2_product.validateLicenseFile(
            kwargs["parsed_args"].licenseFilePath
        )
        if returnCode == 0:
    ...
    def do_validateLicenseStringBase64(self, **kwargs):
        ...
        returnCode = self.g2_product.validateLicenseStringBase64(
            kwargs["parsed_args"].licenseString
        )
        if returnCode == 0:
            self.printResponse("License validated.", "success")
        else:
    ...
    def do_setOutputFormat(self, **kwargs):
        ...
        self.printResponse(
            colorize_msg(
                f"JSON format set to {kwargs['parsed_args'].outputFormat.lower()}",
                "info",
            )
        )
    ...
    def do_setOutputColor(self, **kwargs):
        ...
        if not kwargs["parsed_args"].outputColor:
            self.printResponse(
                colorize_msg(
        ...
        if kwargs["parsed_args"].outputColor.lower() not in ("on", "off"):
            self.printResponse(
                colorize_msg("Color mode should be on or off", "warning")
            )
            return

        if kwargs["parsed_args"].outputColor.lower() == "on":
            self.json_output_color = True
            self.printResponse(colorize_msg("Colored output is enabled", "info"))

        if kwargs["parsed_args"].outputColor.lower() == "off":
            self.json_output_color = False
            self.printResponse(colorize_msg("Colored output is disabled", "info"))
    ...
    def printWithNewLines(self, ln, pos=""):
        ...
        if orjson_avail:
            try:
                ...
                output = do_color(json_str.decode())
        else:
            try:
                _ = json.loads(ln)
            except (json.decoder.JSONDecodeError, TypeError):
    ...
    def complete_hashFile(self, text, line, begidx, endidx):
        if re.match(".* -o +", line):
            ...
            return self.pathCompletes(
                text, line, begidx, endidx, line[: path_start + 2]
            )
