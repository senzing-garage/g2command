    ...
    --FORCEPURGE = Don't prompt before purging. USE WITH CAUTION!\n'''
        ...
        try:
            ...
            if not args.forcePurge:
                if input('\n*** This will purge all currently loaded data from the Senzing repository! Before proceeding all other instances of the Senzin API (custom code, Rest API, redoer, etc.) MUST be shut down. ***\n\nAre you sure? (y/n)  ') not in ['y', 'Y', 'yes', 'YES']:
                    print()
                    return
            else:
